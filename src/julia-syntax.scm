    ...
    (cond ((atom? ex) (expr-contains-eq v ex))
          ...
          ((eq? (car ex) 'method)
           (and (length> ex 2)
                ...
                ;; same function do.
                (let ((all-methods (get-methods ex body)))
                  (any (lambda (ex)
                         (assq v (cadr (lam:vinfo (cadddr ex)))))
                       all-methods))))
          (else (expr-contains-eq v ex))))
  (let* ((vi     (car (lam:vinfo lam)))
         (unused (map car (filter (lambda (v) (and (vinfo:capt v) (vinfo:sa v))) vi)))
         (seen   '())
         (live   '()))
    (define (kill)
      (set! unused (append live unused))
      (set! live '()))
    (define (visit e)
      (cond ((and (pair? e)
                  (memq (car e) '(quote top core line inert local local-def unnecessary
                                        meta inbounds boundscheck simdloop decl
                                        struct_type abstract_type primitive_type thunk
                                        implicit-global global globalref outerref
                                        const null ssavalue
                                        gc_preserve_begin gc_preserve_end)))
             #f)
            ((and (pair? e) (eq? (car e) 'scope-block))
             (visit (cadr e)))
            ((and (pair? e) (or (eq? (car e) 'block) (eq? (car e) 'body)))
             (for-each visit (cdr e)))
            ((and (pair? e) (eq? (car e) 'return))
             (kill))
            ((and (pair? e) (or (eq? (car e) 'if) (eq? (car e) 'elseif)))
             (for-each (lambda (e)
                         (visit e)
                         (kill))
                       (cdr e)))
            (else
             (set! unused (filter (lambda (v) (not (expr-uses-var e v (lam:body lam))))
                                  unused))
             (cond ((and (pair? e) (memq (car e) '(method =)) (memq (cadr e) unused))
                    (begin (set! live (cons (cadr e) live))
                           (set! seen (cons (cadr e) seen))
                           (set! unused (filter (lambda (v) (not (eq? v (cadr e)))) unused))))
                   ((or (atom? e)
                        (memq (car e) '(new = method call foreigncall cfunction)))
                    #t) ;; safe
                   (else
                    (kill))))))
    (visit (lam:body lam))
    (for-each (lambda (v)
                (if (memq v seen)
                    (let ((vv (assq v vi)))
                      (if vv (vinfo:set-never-undef! vv #t)))))
              (append live unused))
